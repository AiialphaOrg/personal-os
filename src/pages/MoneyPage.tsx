...
export function MoneyPage() {
  ...
  const [walletFilter, setWalletFilter] = useState<"all" | "spending" | "savings" | "investments">("all")
  ...
  const investmentWallets = wallets.filter((w) => w.kind === "investment")

  const filteredWallets =
    walletFilter === "spending"
      ? spendingWallets
      : walletFilter === "savings"
      ? savingsWallets
      : walletFilter === "investments"
      ? investmentWallets
      : wallets
        ...
        <section className="space-y-3">
          ...
          <div className="flex items-center justify-between gap-2.5">
            <div className="relative flex-1 max-w-xs">
              <select
                value={walletFilter}
                onChange={(e: any) => setWalletFilter(e.target.value)}
              ...
              >
                <option value="all">All Wallets & Accounts ({wallets.length})</option>
                <option value="spending">Spending ({spendingWallets.length})</option>
                <option value="savings">Savings ({savingsWallets.length})</option>
                <option value="investments">Investments ({investmentWallets.length})</option>
              </select>
